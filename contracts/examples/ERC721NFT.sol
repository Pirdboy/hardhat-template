...
contract ERC721NFT is ERC721URIStorage, ReentrancyGuard {
    ...
    uint256 public maxBalance = 5; // 每个人最多只能mint 5个
    ...
    function mintNFT() external payable nonReentrant canMint returns (uint) {
        tokenId.increment();
        uint currentTokenId = tokenId.current();
        _safeMint(msg.sender, currentTokenId);

        string memory uri = string(abi.encodePacked(currentTokenId.toString(), baseExtension));
